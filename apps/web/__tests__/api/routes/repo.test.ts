...
describe("GET /api/repo", () => {
  ...
  it("should return repositories for authenticated user", async () => {
    ...
    const testReposInResponse = data.repositories.filter(
      (r: { slug: string }) => r.slug.startsWith(testPrefix)
    );

    expect(testReposInResponse.length).toBeGreaterThanOrEqual(2);

    // Verify repository structure
    const repo1 = testReposInResponse.find(
      (r: { slug: string }) => r.slug === `${testPrefix}/repo-1`
    );
  ...
  it("should return 401 when not authenticated", async () => {
    ...
    const request = new NextRequest("http://localhost:3000/api/repo");
  ...
  it("should return repositories with null avatarUrl when not provided", async () => {
    ...
    // Find our test repo
    const testRepo = data.repositories.find(
      (r: { slug: string }) => r.slug === `${testPrefix}/repo-no-avatar`
    );
